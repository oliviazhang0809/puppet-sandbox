...
puppet_nodes = [
  ...
  {:hostname => 'influxdbSeed',   :role => 'influxdbSeed',  :ip => '172.16.32.16', :fwdhost => 8004, :fwdguest => 8083, :autostart => true, :ram => 1024},
  {:hostname => 'grafana',        :role => 'grafana',       :ip => '172.16.32.17', :fwdhost => 8003, :fwdguest => 80,   :autostart => true, :ram => 1024},
  {:hostname => 'influxdbChild1', :role => 'influxdbChild', :ip => '172.16.32.18', :fwdhost => 8005, :fwdguest => 8083, :autostart => true, :ram => 1024},
  {:hostname => 'influxdbChild2', :role => 'influxdbChild', :ip => '172.16.32.19', :fwdhost => 8006, :fwdguest => 8083, :autostart => true, :ram => 1024},
  {:hostname => 'hekad',          :role => 'heka',          :ip => '172.16.32.20', :fwdhost => 8007, :fwdguest => 8125, :autostart => true, :ram => 1024},
]
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  puppet_nodes.each do |node|
    ...
    config.vm.define node[:hostname], autostart: node[:autostart] do |node_config|
      ...
      if node[:role] == "influxdbSeed"
       node_config.vm.provision "shell" do |s|
          s.inline = "mkdir -p /etc/facter/facts.d && echo cluster_seed_servers=`facter fqdn` > /etc/facter/facts.d/cluster_seed_servers.txt"
       end
     end

      if node[:role] == "master"
        node_config.vm.provision "shell" do |s|
          s.inline = "mkdir -p /etc/facter/facts.d && echo puppet_hostname=`facter fqdn` > /etc/facter/facts.d/puppet_hostname.txt"
        end
      end
      ...
      node_config.vm.synced_folder "./custom-facts", "/etc/facter/facts.d", create: true
